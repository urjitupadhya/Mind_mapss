export async function generateRecoverySuggestions(context: {
  ...
  const suggestions: {
    immediate: { type: string; title: string; description: string; duration: string };
    shortTerm: { type: string; title: string; description: string; duration: string };
    strategic: { type: string; title: string; description: string };
  }[] = [];
...
function getRecoveryType(context: {
  ...
  if (context.errorRate > 0.3) return 'cognitive_overload';
  if (context.fileSwitchRate > 0.5) return 'context_switching';
  if (context.strainLevel === 'critical') return 'exhaustion';
  if (context.strainLevel === 'high') return 'fatigue';
  if (context.burnoutProbability > 50) return 'burnout_risk';
  return 'maintenance';
}
...
function generatePersonalizedMessage(context: {
  ...
  else if (hour >= 12 && hour < 14) timeContext = 'post-lunch';
  else if (hour >= 9 && hour < 12) timeContext = 'morning';
  else timeContext = 'afternoon';
...
function getEstimatedRecoveryTime(context: {
  ...
  if (context.strainLevel === 'critical') return '30-60 minutes';
  if (context.strainLevel === 'high') return '15-30 minutes';
  if (context.strainLevel === 'moderate') return '5-15 minutes';
