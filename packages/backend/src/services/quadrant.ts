export function calculateProductivityQuadrant(bpiMetrics: Array<{
...
export function calculateWeeklyBPI(bpiMetrics: Array<{
  date: string;
  avg_cognitive_score: number;
  avg_burnout_probability: number;
  avg_flow_state: number;
}>) {
  if (!bpiMetrics || bpiMetrics.length === 0) {
    return {
      weeklyBPI: 0,
  ...
  const weeklyBPI = Math.round(
    bpiMetrics.reduce((sum, d) => sum + (d.avg_burnout_probability || 0), 0) / bpiMetrics.length
  );
  ...
  if (bpiMetrics.length >= 3) {
    ...
    if (recentAvg > earlierAvg + 10) trend = 'worsening';
    else if (recentAvg < earlierAvg - 10) trend = 'improving';
    else trend = 'stable';
  } else {
    trend = 'stable';
  }
