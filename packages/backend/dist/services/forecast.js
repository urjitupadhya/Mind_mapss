...
export function generateForecast(historicalData) {
    // Group by hour
    ...
    for (let i = 0; i < 24; i++) {
        ...
        if (hourData && hourData.samples >= 3) {
            ...
            riskScore = Math.round((normalizedErrorRate * 0.35 +
...
function getBaselineStability(hour) {
...
function getContributingFactors(hour, riskScore) {
    const factors = [];
    if (hour >= 22 || hour < 6) {
...
function findOptimalWindow(forecasts) {
...
function findHighRiskWindows(forecasts) {
    const windows = [];
    let currentStart = -1;
    let currentReason = '';
    for (let i = 0; i < forecasts.length; i++) {
