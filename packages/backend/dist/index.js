...
import { calculateProductivityQuadrant } from './services/quadrant';
...
fastify.get('/api/gamification/:userId', async (request, reply) => {
    ...
    try {
        ...
        const badges = [];
        if (gamificationStats.totalSessions >= 1)
            badges.push('first_session');
        if (gamificationStats.currentStreak >= 3)
        ...
        if (gamificationStats.earlySessions > 0)
            badges.push('early_bird');
        if (gamificationStats.lateSessions > 0)
            badges.push('night_owl');
        if (gamificationStats.peakFlow >= 80)
            badges.push('flow_state');
        if (gamificationStats.lowStreakMinutes >= 120)
            badges.push('low_strain');
        if (gamificationStats.longestSession >= 180)
            badges.push('deep_work');
        if (gamificationStats.daysTracked >= 14)
