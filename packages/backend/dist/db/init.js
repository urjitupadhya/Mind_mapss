...
export function upsertHourlyAggregate(db, userId, data) {
  ...
  `).run(data.userId || userId, data.hour, data.date, data.avgStability, data.errorRate, data.complexityIndex, data.typingCadenceVariance, data.undoSpikes, data.fileSwitches, data.sessionCount, data.totalDurationMinutes, Date.now());
}
...
export function saveForecast(db, userId, data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(userId, data.hour, data.date, data.predictedStability, data.riskScore, data.confidence, JSON.stringify(data.contributingFactors), Date.now());
}
...
export function saveInsight(db, userId, data) {
    ...
    VALUES (?, ?, ?, ?, ?)
  `).run(userId, data.insightType, data.content, data.dataPayload ? JSON.stringify(data.dataPayload) : null, Date.now());
}
...
export function saveBPIMetrics(db, userId, data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(userId, data.date, data.hour, data.cognitiveScore, data.burnoutProbability, data.flowState, data.emaCognitiveScore, data.emaStability, data.weightedBreakdown || null, data.normalizedMetrics || null, Date.now());
}
...
export function saveUserPreferences(db, userId, prefs) {
    ...
    if (existing) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(userId, prefs.codingStartHour || 9, prefs.codingEndHour || 18, prefs.sleepStartHour || 22, prefs.sleepEndHour || 6);
    }
}
